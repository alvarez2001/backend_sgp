...
export abstract class BaseReadRepository<T> {
  async findByCriteria(
    ...
    const queryData: any = JSON.parse(
  ...
  private buildQueryObject(filters: { [key: string]: any }): any {
  ...
  private parseSortCriteria(sort?: string): any {
    const sortObject = {};
    if (sort) {
  ...
  private isNumberValid(value: any) {
