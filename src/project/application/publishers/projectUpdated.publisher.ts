...
import { ProjectUpdatedConsumer } from '../consumers/projectUpdated.consumer';

@Injectable()
export class ProjectUpdatedPublisher {
    constructor(private readonly rabbitMQService: RabbitMQService) {
        appEvents.on(DeclarationQueues.project_updated, async project => {
    ...
    async send(data: Project): Promise<void> {
        const model = {
            ...
            displayNames: {
                [DeclarationQueues.project_updated]: ProjectUpdatedConsumer.name,
            },
        };
