...
export const AUTHENTICATION_READ_REPOSITORY_INTERFACE = 'AuthenticationReadRepositoryInterface';
...
export interface AuthenticationReadRepositoryInterface {
    ...
    update(id: number, authentication: UpdateAuthenticationDto): Promise<AuthenticationRead>;

    findById(id: number): Promise<AuthenticationRead | null>;

    findAll(): Promise<AuthenticationRead[]>;

    pagination(criteria: SearchCriteriaDto): Promise<PaginateResponseDto<AuthenticationRead>>;
