...
export class AuthenticationService {
  ...
  async signIn(
    ...
    if (!user) throw new UnauthorizedException({ msg: 'Invalid credentials' });
    ...
    let authentication = await this.authenticationRepository.findByUserId(
      user.id,
    );
    ...
    const expiresIn = parseInt(this.configService.get('HOURS_JWT')) * 60 * 60;
    ...
    authentication = await this.authenticationRepository.findById(
      authentication.id,
    );
